...
This script performs project setup and FASTQ discovery only. It does not copy
FASTQs, run mapping, or run downstream scientific calculations.

Paired-end sequencing layout is recorded separately from genome mapping
strategy. For the Zn toxicity project the manually validated default is paired
end input with single-mate R1 genome mapping, but the manifest fields remain
explicit so future projects can request R2 or paired-genome mapping.
"""
...
MANIFEST_COLUMNS = [
    ...
    "fastq_1", "fastq_2", "layout", "sequencing_layout", "mapping_strategy",
    "genome_mapping_mate", "junction_mate", "require_mate_alignment",
]

SAMPLE_SHEET_COLUMNS = [
    "sample", "fastq_1", "fastq_2", "layout", "sequencing_layout",
    "mapping_strategy", "genome_mapping_mate", "junction_mate",
    "require_mate_alignment", "condition", "condition_label", "background",
    "pool", "pool_id", "replicate_id", "treatment", "treatment_label",
    "control_or_treated", "library_role", "classifier_role", "fitness_role",
    "species", "hit_file", "guessed_condition", "guessed_background",
    "guessed_pool", "include", "warnings",
]

COMPARISON_COLUMNS = [
    "comparison_id", "pair_id", "display_comparison", "comparison_group",
    "comparison_label", "control_sample", "treated_sample", "parent_sample",
    "background", "treatment", "treatment_label", "treatment_concentration_mM",
    "parent_condition", "control_condition", "treated_condition", "pool",
    "pool_id", "replicate_id", "original_background", "include", "included",
    "status", "notes",
]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--mapping-strategy",
        choices=("single_mate", "paired_genome"),
        default="single_mate",
        help="Genome mapping strategy. Zn toxicity defaults to validated single-mate R1 mapping.",
    )
    parser.add_argument(
        "--genome-mapping-mate",
        choices=("R1", "R2"),
        default="R1",
        help="Mate used for single_mate genome mapping.",
    )
    parser.add_argument(
        "--junction-mate",
        choices=("R1", "R2"),
        default="R1",
        help="Mate containing the transposon/genome junction signal.",
    )
    parser.add_argument(
        "--require-mate-alignment",
        action="store_true",
        help="Require both mates to align for strategies that support mate-alignment requirements.",
    )
...
def infer_metadata(sample: str) -> dict[str, str]:
    pool_match = re.search(r"pool([0-9]+)", sample)
    ...
    if "1_5mM-Zn-treated" in sample or "Zn-treated" in sample:
        return {
...
def make_comparisons(rows: list[dict[str, str]]) -> list[dict[str, str]]:
    ...
    for pool_id in sorted(by_pool, key=lambda value: int(value) if value.isdigit() else value):
...
def validate_mapping_args(args: argparse.Namespace) -> None:
    if args.read_layout == "paired_end" and not args.mapping_strategy:
        fail("paired-end input requires explicit --mapping-strategy")
    if args.mapping_strategy == "single_mate" and not args.genome_mapping_mate:
        fail("single_mate mapping requires explicit --genome-mapping-mate")
    if args.mapping_strategy == "paired_genome" and args.require_mate_alignment:
        # This is permitted, but make the manifest intent explicit rather than
        # coupling it to sequencing layout.
        return


def main() -> int:
    args = parse_args()
    validate_mapping_args(args)
    ...
    for row in submission_rows:
        ...
        manifest_rows.append({
            ...
            "fastq_2": str(r2),
            "layout": "paired",
            "sequencing_layout": args.read_layout,
            "mapping_strategy": args.mapping_strategy,
            "genome_mapping_mate": args.genome_mapping_mate,
            "junction_mate": args.junction_mate,
            "require_mate_alignment": bool_string(args.require_mate_alignment),
            **meta,
        })
    ...
    for row in manifest_rows:
        sample_rows.append({
            ...
            "genome_mapping_mate": row["genome_mapping_mate"],
            "junction_mate": row["junction_mate"],
            "require_mate_alignment": row["require_mate_alignment"],
    ...
    print(
        "Paired-end handling: R1 and R2 FASTQs are recorded, sequencing_layout "
        f"is {args.read_layout}, mapping_strategy is {args.mapping_strategy}, "
        f"genome_mapping_mate is {args.genome_mapping_mate}, junction_mate is "
        f"{args.junction_mate}, require_mate_alignment is "
        f"{bool_string(args.require_mate_alignment)}. Paired-end input is not "
        "automatically passed to MapFastq as paired-genome alignment."
    )

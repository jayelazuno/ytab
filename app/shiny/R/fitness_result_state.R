...
fitness_result_table_data <- function(data, repo_root = NULL){normalized<-normalize_fitness_result_columns(data);wanted<-c("feature_id","gene","fitness_call","mean_log2fc","min_z","max_z","comparisons_depleted","comparisons_enriched","classifier_label");visible<-intersect(wanted,names(normalized$data));if(!length(visible))return(data.frame());x<-normalized$data[,visible,drop=FALSE];if(!is.null(repo_root)&&"feature_id"%in%names(x)){mapped<-ytab_glabrata_regular_display_columns(x,repo_root,"feature_id");x<-cbind(mapped,x[,setdiff(names(x),c("feature_id","gene")),drop=FALSE])};labels<-c(feature_id="Feature ID",gene="Gene",fitness_call="Fitness call",mean_log2fc="Mean log2 fold change",min_z="Minimum z-score",max_z="Maximum z-score",comparisons_depleted="Comparisons depleted",comparisons_enriched="Comparisons enriched",classifier_label="Classifier label");names(x)<-ifelse(names(x)%in%names(labels),unname(labels[names(x)]),names(x));x}

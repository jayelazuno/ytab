...
`ytab_setup_env.sh` creates or updates the `ytab-local` environment from
...
Before starting a development session, collaborators should sync from the
current shared branch with:

```bash
./scripts/local/ytab_sync_start.sh
```

This script refuses to run if local staged, unstaged, or untracked files exist,
